...
1. **Per-opponent scouting dossiers** — roster with age range, NTRP (+ rating type C/S/A/D/M), WTN singles/doubles, TennisRecord dynamic rating; season singles/doubles records (**superseded 2026-08-03, issue #90**: the spec said last-six-months, which made the binder thinnest on the day it is used — a spring season is largely out of a rolling six-month window by an August tournament; **further superseded 2026-08-09, issue #122**: #90's calendar-year season was ALSO the wrong axis — several sections play their qualifying season in a different calendar year than an August tournament, which silently deleted two of five Sectionals teams' records from the binder. Replaced by a 12-month lookback anchored to the same event, which the #90 fix already anchored to correctly); results within this tournament; court-assignment tendencies; partner frequency; prior meetings vs. our players; and a **predicted lineup honestly labeled a guess**.
...
Same services; tool names mirror the grammar (`team_pull`, `player_show`, `lineup_plan`, …). Agent chat is the analysis/planning surface and where captain notes are captured conversationally. **Protocol revision: v1 speaks `2025-11-25` via `@modelcontextprotocol/sdk` v1, and deliberately does not track the `2026-07-28` revision or the v2 SDK** ([#106](https://github.com/wrburgess/nadal/issues/106)). Declined on **inapplicability, not on cost alone** — all four entries in the revision's deprecation list (Roots/Sampling/Logging; HTTP+SSE; the `includeContext` values `"thisServer"`/`"allServers"`; OAuth 2.0 Dynamic Client Registration) miss this surface, so nothing here is deprecated or broken. What it *adds* splits two ways and the distinction matters: the `Mcp-Session-Id` header, `subscriptions/listen` and SSE resumability are Streamable-HTTP-only and never reach a stdio server, while the removal of **protocol-level** sessions (list endpoints no longer vary per connection), statelessness, the required `resultType`, `CacheableResult` and MRTR are **protocol-wide and do apply here** — they are declined not because they miss this transport but because nadal needs none of them (`MCP_TOOLS` is a static array whose list results already do not vary per connection; it initiates no server requests; it mints no error codes; and it has one client that is not polling a cache). The cost side is real but secondary: the migration rewrites every tool registration and re-opens `src/cli/commands/mcp-serve.ts`, whose stdin-EOF handling was established empirically against v1 transport internals and would have to be re-derived by running it against v2's rewritten transport. nadal's half of the compatibility story is measured and pinned in `test/mcp-protocol-negotiation.test.ts` — a stateless request that never sends `initialize` is answered with the full tool list, an unrecognized `protocolVersion` is downgraded rather than refused, and the unimplemented `server/discover` probe returns a recoverable `-32601`. Explicitly *not* claimed — that any particular `2026-07-28` client will therefore work; honoring a fallback is a **client** obligation this project cannot guarantee, and only the client half is unmeasured. One concrete residual is named rather than smoothed over: the revision supplies its own backward-compatibility rule for the missing `resultType` (clients MUST read an omitted field as `"complete"`) but states none for `CacheableResult`'s `ttlMs`/`cacheScope` on `tools/list`, so a client validating list results strictly could refuse a v1 reply — which is why this ships with a trigger rather than a compatibility claim. The trigger that reopens it is one observation — a client listing **zero** tools against `tn mcp serve` — and is documented where an operator meets it, in [agent-chat-over-mcp.md](../../runbooks/agent-chat-over-mcp.md). Note that no dependency signal exists: v2 ships under three new package names, so `@modelcontextprotocol/sdk` stays undeprecated at `latest` and `npm outdated` will never raise this.
